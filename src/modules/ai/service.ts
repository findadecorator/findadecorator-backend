export function suggestQuoteAssistant(input: { jobDescription: string; projectType?: string }) {
...
export function suggestColourAdvisor(input: { roomPhotoUrl?: string; roomStyle?: string }) {
...
export function classifyJob(input: { jobDescription: string }) {
  const text = (input.jobDescription ?? "").toLowerCase();
  if (text.includes("villa") || text.includes("large") || text.includes("whole house")) return { classification: "large", confidence: 0.92 };
  if (text.includes("kitchen") || text.includes("bathroom") || text.includes("two rooms")) return { classification: "medium", confidence: 0.84 };
