...
exports.topUpCredits = topUpCredits;
...
exports.previewLead = previewLead;
exports.unlockLead = unlockLead;
exports.requestRefund = requestRefund;
exports.listRefunds = listRefunds;
exports.getCreditBalances = getCreditBalances;
const engine_1 = require("../../../pricing-engine/engine");
const versioning_1 = require("../../../pricing-engine/versioning");
const leads = new Map();
const unlocks = new Map();
const unlockByIdempotencyKey = new Map();
const unlockByOwnerLead = new Map();
const ledgerEntries = [];
const refunds = [];
const subscriptionCreditsByProfessional = new Map();
const walletByProfessional = new Map();
function makeId(prefix) {
    return `${prefix}_${Math.random().toString(36).slice(2, 10)}`;
}
function evaluateEligibility(input) {
    const reasons = [];
    if (input.professionalRating < 3)
        reasons.push("professional-rating-too-low");
    if (input.valueAnchorGbp > 6000)
        reasons.push("manual-review-required");
    return { eligible: reasons.length === 0, reasons };
}
function appendLedger(professionalId, deltaCredits, reason) {
    const next = (walletByProfessional.get(professionalId) ?? 0) + deltaCredits;
    walletByProfessional.set(professionalId, next);
    const entry = { id: makeId("ledger"), professionalId, deltaCredits, source: "wallet", reason, createdAt: new Date().toISOString() };
    ledgerEntries.push(entry);
    return entry;
}
function appendSubscriptionLedger(professionalId, deltaCredits, reason) {
    ...
    const entry = { id: makeId("ledger"), professionalId, deltaCredits, source: "subscription", reason, createdAt: new Date().toISOString() };
    ledgerEntries.push(entry);
    return entry;
}
function topUpCredits(professionalId, credits, reason = "manual-topup") {
    return appendLedger(professionalId, credits, reason);
}
function grantSubscriptionCredits(professionalId, credits, reason = "subscription-plan") {
    return appendSubscriptionLedger(professionalId, credits, reason);
}
function getWallet(professionalId) {
...
function listLedger(professionalId) {
...
function unlockLeadPayload(lead) {
    return {
        id: lead.id,
        title: lead.title,
        description: lead.description,
...
function createLead(input) {
    const leadClass = (0, engine_1.classifyLeadFromAnchor)(input.valueAnchorGbp);
    const pricingSnapshot = (0, engine_1.calculateDeterministicLeadPrice)({
    ...
    const lead = {
        ...
        pricingSnapshot: {
            ...pricingSnapshot,
            pricingRuleVersion: (0, versioning_1.getActivePricingRuleVersion)().version
        },
...
function previewLead(leadId, professionalId) {
...
function unlockLead(input) {
    ...
    if (!lead)
        throw new Error("Lead not found");
    if (!lead.eligibility.eligible)
        throw new Error(`Lead not eligible: ${lead.eligibility.reasons.join(", ")}`);
...
function requestRefund(unlockId, reason) {
    const unlock = unlocks.get(unlockId);
    if (!unlock)
        throw new Error("Unlock not found");
    const refund = { id: makeId("refund"), unlockId, reason, status: "pending", createdAt: new Date().toISOString() };
    refunds.push(refund);
    return refund;
}
...
function getCreditBalances(professionalId) {
